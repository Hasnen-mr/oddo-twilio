#!/usr/bin/env bash
# MCP Claude Integration for Odoo — macOS / Linux installer
# Finds Odoo, extracts the corresponding version package into custom addons, installs Python dependencies.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
if [[ -f "$SCRIPT_DIR/../__manifest__.py" ]]; then
  MODULE_SRC="$(cd "$SCRIPT_DIR/.." && pwd)"
...
echo
echo "=============================================="
echo "  MCP Claude Integration Installer (macOS / Linux)"
echo "=============================================="
echo

# ── Step 1: locate Odoo / addons ─────────────────────────────────────────────
info "Step 1/4 — Looking for Odoo installation directories..."
...
ok "Target Odoo Version: Odoo $ODOO_VERSION"

ZIP_FILE=""
ZIP_CANDIDATES=()

ZIP_CANDIDATES+=("$SCRIPT_DIR/../packages/mcp_claude_$ODOO_VERSION.0.zip")
ZIP_CANDIDATES+=("$SCRIPT_DIR/packages/mcp_claude_$ODOO_VERSION.0.zip")
ZIP_CANDIDATES+=("$SCRIPT_DIR/mcp_claude_$ODOO_VERSION.0.zip")
ZIP_CANDIDATES+=("$REPO_ROOT/release/packages/mcp_claude_$ODOO_VERSION.0.zip")
ZIP_CANDIDATES+=("$REPO_ROOT/packages/mcp_claude_$ODOO_VERSION.0.zip")
ZIP_CANDIDATES+=("$REPO_ROOT/mcp_claude_$ODOO_VERSION.0.zip")
...
info "Step 2/4 — Installing MCP Claude module into custom addons..."
TARGET="$ADDONS_DIR/mcp_claude"

if [[ -e "$TARGET" ]]; then
  warn "Existing module found at $TARGET [UPGRADE MODE]"
  read -r -p "Replace existing installation? [y/N]: " repl
  if [[ "${repl:-N}" =~ ^[Yy]$ ]]; then
    BACKUP_DIR="$ADDONS_DIR/mcp_claude_backup_$(date +%Y%m%d%H%M%S)"
...
if [[ -n "$ZIP_FILE" && -f "$ZIP_FILE" ]]; then
  info "Extracting release package: $ZIP_FILE → $ADDONS_DIR"
  unzip -q -o "$ZIP_FILE" -d "$ADDONS_DIR"

  if [[ -d "$TARGET/mcp_claude" && -f "$TARGET/mcp_claude/__manifest__.py" ]]; then
    info "Resolving nested directory structure..."
    mv "$TARGET/mcp_claude" "$ADDONS_DIR/mcp_claude_temp"
    rm -rf "$TARGET"
    mv "$ADDONS_DIR/mcp_claude_temp" "$TARGET"
  fi
elif [[ -d "$MODULE_SRC" && -f "$MODULE_SRC/__manifest__.py" ]]; then
  info "Copying module source: $MODULE_SRC → $TARGET"
  cp -R "$MODULE_SRC" "$TARGET"
else
  fail "Could not find MCP Claude release package for Odoo $ODOO_VERSION"
fi

# Verification Check
[[ -f "$TARGET/__manifest__.py" ]] || fail "Verification Error: __manifest__.py not found at $TARGET/__manifest__.py"
[[ ! -f "$TARGET/mcp_claude/__manifest__.py" ]] || fail "Verification Error: Double nesting detected at $TARGET/mcp_claude/__manifest__.py"
...
echo
echo "${GREEN}==========================================================${NC}"
echo "${GREEN}  MCP Claude Integration has been installed into Odoo Apps.${NC}"
echo "${GREEN}==========================================================${NC}"
echo
echo "${YELLOW}Next steps:${NC}"
echo "  1. Restart your Odoo server."
echo "  2. Open Odoo in your web browser."
echo "  3. Go to Settings -> Activate Developer Mode."
echo "  4. Open Apps menu."
echo "  5. Click \"Update Apps List\" in top navigation bar."
echo "  6. Search for \"MCP Claude\"."
echo "  7. Click \"Activate\" (or Install) to enable the module."
echo "  8. Open MCP Claude -> Control Center to configure your API keys."
echo
