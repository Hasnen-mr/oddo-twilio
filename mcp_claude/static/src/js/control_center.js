...
export class MCPControlCenter extends Component {
    ...
    async sendRegistrationData(isResend = false) {
        ...
        if (dummyDomains.includes(domain) || domain.includes("example.") || domain.includes(".example") || domain.includes("yourcompany.")) {
            this.state.otpError = "Please enter a real work or personal email address (example and test domains are not supported).";
            return;
        }
        if (["example", "dummy", "fake", "tempmail", "testmail"].some(k => userPart.includes(k)) && (domain.includes("gmail") || domain.includes("yahoo") || domain.includes("outlook") || domain.includes("hotmail"))) {
            this.state.otpError = "Please enter a real work or personal email address (example and dummy email addresses are not supported).";
            return;
        }
